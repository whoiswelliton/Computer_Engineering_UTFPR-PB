...
void erro_horizontal(void)
{
    if(processar_LDR)
    {
       processar_LDR = 0;
        ...
        erro = LDR_DIREITA - LDR_ESQUERDA;

        if(erro > ErroH)
        {
            if(TA1CCR1 > 1500)
            {
                TA1CCR1 -= 20;   //movedireita
            }
        }
        else if(erro < -ErroH)
        {
...
void ini_Timer1_PWM(void)
{
     ...
     *Quantas contagens para 0.9ms? >> TA0CCR1 = (2MHz * 0.0009) - 1 = 1799 --- valor min
     *Quantas contagens para 2.1ms? >> TA0CCR1 = (2MHz * 0.0021) - 1 = 4199 --- valor max
     */
    TA1CTL = TASSEL1 + MC0;
    TA1CCTL0 = CCIE;
